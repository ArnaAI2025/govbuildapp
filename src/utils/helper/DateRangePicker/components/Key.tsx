...
const Key: FC<KeyProps> = ({ colorOptions, Day, mode, output, setOutput }: KeyProps) => {
  const { dateTextColor, backgroundColor, selectedDateTextColor, selectedDateBackgroundColor } =
    colorOptions;
  ...
  const onKeyPress = () => {
    ...
    const shouldSetStartDate =
      !output.startDate || output.endDate || newDate.getTime() < output.startDate?.getTime();
  ...
  const getColor = () => {
    ...
    const isThisDayInSelectedRange =
      timeOfThisKey >= startDate.getTime() && timeOfThisKey <= output.endDate.getTime();
